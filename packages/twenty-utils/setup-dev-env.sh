...
redis_is_up() {
  if command -v redis-cli &>/dev/null; then
    redis-cli -h localhost -p 6381 ping 2>/dev/null | grep -q PONG
    ...
    timeout 2 bash -c 'exec 3<>/dev/tcp/localhost/6381; echo PING >&3; read -r reply <&3; exec 3>&-; echo "$reply"' 2>/dev/null | grep -q PONG
  fi
}
...
ok "PostgreSQL on localhost:5435"
ok "Redis on localhost:6381"
