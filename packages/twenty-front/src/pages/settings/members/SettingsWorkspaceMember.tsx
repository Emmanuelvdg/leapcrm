...
export const SettingsWorkspaceMember = () => {
  ...
  const subscriptionStatus = subscriptionStatusData?.mySubscriptionStatus;
  const hasSeatCap =
    subscriptionStatus?.status === WorkspaceSubscriptionStatus.ACTIVE ||
    subscriptionStatus?.status === WorkspaceSubscriptionStatus.TRIALING;
  ...
  const handleDeleteMember = async () => {
    ...
    try {
      ...
      if (hasSeatCap && isDefined(subscriptionStatus)) {
        const seatsFreeAfterRemoval =
          subscriptionStatus.seats -
          (subscriptionStatus.activeMembers - 1);

        if (seatsFreeAfterRemoval > 0) {
          enqueueInfoSnackBar({
            message: t`You now have ${seatsFreeAfterRemoval} free seat${seatsFreeAfterRemoval === 1 ? '' : 's'} available for a new invite.`,
          });
        }
      }
