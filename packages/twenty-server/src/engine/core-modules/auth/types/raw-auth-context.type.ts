...
export type RawAuthContext = {
  ...
  // When the user last proved their identity with real credentials. A JWT's
  // own iat is just its renewal time, so this is sourced from either the
  // session (cookie path) or the JWT's own authenticatedAt claim (Bearer
  // path), which is carried forward unchanged across refresh-token rotations
  // rather than reset on renewal — see RefreshTokenJwtPayload.
