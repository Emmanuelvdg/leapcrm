...
export class ConfigVariables {
  ...
  EMAIL_FROM_NAME = 'Felix from Leap CRM';
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SUBSCRIPTION_CONFIG,
    description: 'Enable or disable the per-seat subscription paywall',
    type: ConfigVariableType.BOOLEAN,
  })
  @IsOptional()
  IS_SUBSCRIPTION_BILLING_ENABLED = false;

  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SUBSCRIPTION_CONFIG,
    isSensitive: true,
    description: 'Stripe API key for the per-seat subscription paywall',
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  @ValidateIf((env) => env.IS_SUBSCRIPTION_BILLING_ENABLED === true)
  SUBSCRIPTION_STRIPE_API_KEY: string;

  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SUBSCRIPTION_CONFIG,
    isSensitive: true,
    description: 'Stripe webhook signing secret for the subscription paywall',
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  @ValidateIf((env) => env.IS_SUBSCRIPTION_BILLING_ENABLED === true)
  SUBSCRIPTION_STRIPE_WEBHOOK_SECRET: string;

  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SUBSCRIPTION_CONFIG,
    description:
      'Stripe publishable key for the subscription paywall, exposed to the frontend',
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  @ValidateIf((env) => env.IS_SUBSCRIPTION_BILLING_ENABLED === true)
  SUBSCRIPTION_STRIPE_PUBLISHABLE_KEY: string;

  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SUBSCRIPTION_CONFIG,
    description:
      'Number of trial days a workspace gets before the subscription paywall applies',
    type: ConfigVariableType.NUMBER,
  })
  @CastToPositiveNumber()
  @IsOptional()
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SUBSCRIPTION_CONFIG,
    description: 'Currency used for the per-seat subscription price',
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SUBSCRIPTION_CONFIG,
    description:
      'Workspaces created before this ISO date are permanently exempt from the subscription paywall (grandfathering pre-existing customers when the paywall is introduced)',
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  SUBSCRIPTION_GRANDFATHER_BEFORE: string;
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description: 'LeapCRM server version',
    type: ConfigVariableType.STRING,
    isEnvOnly: true,
    isHiddenInAdminPanel: true,
  })
