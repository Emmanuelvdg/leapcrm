...
export class WorkspaceInvitationService {
  ...
  async sendInvitations(
    ...
    if (emails.length > 0) {
      const pendingInvitationsCount = await this.countPendingInvitations(
        workspace.id,
      );

      await this.workspaceSubscriptionService.assertSeatAvailable({
        workspaceId: workspace.id,
        additionalSeatsNeeded: pendingInvitationsCount + emails.length,
      });
    }
    ...
    for (const invitation of invitationResults) {
      if (invitation.status === 'fulfilled') {
        const link = this.domainServerConfigService.buildBaseUrl({
        ...
        const joinTeamMsg = msg`Join your team on LeapCRM`;
        const i18n = this.i18nService.getI18nInstance(sender.locale);
        const subject = i18n._(joinTeamMsg);

        await this.emailService.send({
          from: `${sender.name.firstName} ${sender.name.lastName} (via Leap CRM) <${this.twentyConfigService.get('EMAIL_FROM_ADDRESS')}>`,
