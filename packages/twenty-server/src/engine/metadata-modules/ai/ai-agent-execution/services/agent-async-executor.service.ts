  ...
  async executeAgent({
    ...
    try {
      if (agent) {
        ...
        if (workspace) {
          await this.aiModelRegistryService.validateModelAvailabilityForWorkspace(
            agent.modelId,
            workspace,
            workspaceId,
          );
      ...
      const registeredModel =
        await this.aiModelRegistryService.resolveModelForAgentForWorkspace(
          agent,
          workspaceId,
        );
      ...
      if (agentSchema) {
        // OpenAI's strict structured-outputs mode (now requested via
        // supportsStructuredOutputs on the openai-compatible provider) rejects
        // any object-typed schema that doesn't explicitly set
        // additionalProperties: false, so backfill it rather than requiring
        // every stored agent schema to set it themselves.
        const strictAgentSchema = isDefined(agentSchema.additionalProperties)
          ? agentSchema
          : { ...agentSchema, additionalProperties: false as const };
        ...
        const structuredResult = await generateText({
          ...
          output: Output.object({ schema: jsonSchema(strictAgentSchema) }),
          providerOptions: getCallLevelProviderOptions({
            sdkPackage: registeredModel.sdkPackage,
            providerOptions: undefined,
            promptCacheKey: agent?.id,
      ...
      const tokenCostInDollars = await this.aiBillingService.calculateCost(
        resolvedModelId,
      ...
      const costInDollars = await this.aiBillingService.calculateCost(
        modelId,
        { usage: accumulatedUsage, cacheCreationTokens },
        workspaceId,
      );
