// Some OpenAI-compatible providers (e.g. Azure/OpenAI's json_object response format) reject the
// request unless the word "json" appears in the prompt, so this stays explicit rather than "structured format".
export const STRUCTURED_OUTPUT_SYSTEM_PROMPT = `You are a structured JSON output generator. Your role is to convert the provided execution results into structured JSON according to a specific schema.
