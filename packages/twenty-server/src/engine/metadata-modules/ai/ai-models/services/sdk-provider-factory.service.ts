...
export class SdkProviderFactoryService {
  ...
  private buildOpenAiCompatibleProvider(
    ...
    const provider = createOpenAICompatible({
      ...
      ...(config.apiKey && { apiKey: config.apiKey }),
      // Without this, the AI SDK sends the older `response_format: json_object`
      // mode, which doesn't enforce the schema shape (the model may nest fields
      // however it likes) and is rejected outright by OpenAI/Azure-routed
      // providers unless the literal word "json" appears in the prompt.
